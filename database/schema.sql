-- database/schema.sql
-- Run this file to set up the PhoneTracker database from scratch.
-- Compatible with MySQL 5.7+ / MariaDB 10.3+

CREATE DATABASE IF NOT EXISTS phonetracker
...
-- ─────────────────────────────────────────────────────────────
-- USERS TABLE
-- Stores all app users (both trackers and tracked devices)
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- USER JOINS TABLE
-- Links parent (watcher) with child (tracked device)
-- ─────────────────────────────────────────────────────────────
...
-- GPS location records uploaded by the tracked device
-- NOTE: Field name typos (lattitude, phone_bettery) are
-- intentional to match the Android app's JSON field names exactly.
-- ─────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS user_locations (
  id                   INT           NOT NULL AUTO_INCREMENT PRIMARY KEY,
  user_id              INT           NOT NULL,
  lattitude            VARCHAR(50)   NOT NULL COMMENT 'Intentional typo — matches Android app',
  longitude            VARCHAR(50)   NOT NULL,
  address              VARCHAR(1000) DEFAULT '',
  phone_bettery        INT           DEFAULT 0 COMMENT 'Intentional typo — matches Android app',
  user_speed           VARCHAR(50)   DEFAULT '0',
  course               FLOAT         DEFAULT 0   COMMENT 'Bearing in degrees',
  accuracy             FLOAT         DEFAULT 0,
  phone_battery_status VARCHAR(20)   DEFAULT 'active',
  is_mock              TINYINT(1)    DEFAULT 0,
  hold_status          VARCHAR(20)   DEFAULT 'moving' COMMENT '"moving" or "stopped"',
  today_date           VARCHAR(50)   DEFAULT '' COMMENT 'UTC datetime string from device',
  created_at           DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
-- Safe or red zones defined by a parent for a child user
-- ─────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS geofence_zones (
  ...
  zone_meter      VARCHAR(20)   NOT NULL COMMENT 'Radius in meters',
...
-- ─────────────────────────────────────────────────────────────
-- NOTIFICATIONS TABLE
-- Stores SOS and other push notifications sent between users
-- ─────────────────────────────────────────────────────────────
...
-- Stores daily app usage stats synced from child devices
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- APP CONFIG TABLE
-- Stores app version info and privacy policy
-- ─────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS app_config (
  id                 INT          NOT NULL AUTO_INCREMENT PRIMARY KEY,
  app_version        VARCHAR(20)  NOT NULL DEFAULT '1.0',
  version_status     VARCHAR(20)  NOT NULL DEFAULT 'ok' COMMENT '"ok" or "update"',
